...
conducted by the National Foundation for Infantile Paralysis (NFIP). It
is adapted from data in the article by Francis et al. (1955). There were
actually two clinical trials, corresponding to two statistical designs
(`Experiment`), discussed by Brownlee (1955). The comparison of designs
...
- `Group`:
  ...
  `IncompleteVaccinations` `NotInoculated` `Placebo` `Vaccinated`
...
selected (again in various schools in various areas), all of whose
