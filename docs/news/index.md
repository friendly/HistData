...
- Add `Federalist` data: `Federalist_corpus`, `Federalist_markers`, and
  ...
  Mosteller & Wallace-style authorship-attribution and text analysis
...
This is a maintenance release, largely fixing CRAN check problems since
the last public release, 0.9-1
...
- Added `Mayer` and `Saturn` data sets \[thx: Luiz Fernando Palin
  Droubi\]
...
- Added `ChestStigler` data, Stigler’s correction of Quetelet
  `ChestSizes` (now depends on R (\>= 3.5.0))
...
- Add clarifying note to Arbuthnot.Rd that the data are christenings,
  not births (thx: Jim Oeppen <joeppen@health.sdu.dk>)

## Version 0.8-2 (2017-02-10)

CRAN release: 2017-08-16

- Added Herschel’s data on the twin stars Virginis, used in the first
  scatterplot
...
- Added SnowMap()
- fixed Snow.pumps data: label should be character, not a factor; added
  missing labels for pumps 1:2
- SnowMap now uses asp=1
- Replaced Snow.polygons with new \## Version consisting of actual
  vertices of the polygons surrounding each pump.
...
- Substantially revised examples in Snow.Rd to make plots more flexible.
  [`SnowMap()`](https://friendly.github.io/HistData/reference/SnowMap.md)
  should be a function in the package and made more general, but this is
  not yet done.

## Version 0.7-8 (2016-04-13)

CRAN release: 2016-04-13
...
- Fixed Nightingale.Rd examples to avoid obscuring some segments by
  overplotting \[thx: Monique Graf\].
...
- CushnyPebbles renamed to proper spelling, CushnyPeebles (thx: Ian E.
  Holliday)
- Removed Suggests: ReadImage; for now, retain old code examples as
- Extended GaltonFamilies examples

## Version 0.7-0

CRAN release: 2012-12-31
...
- Replaced original data(Wheat) on 26 intervals with a new \## Version
  at 53 time points and a separate data.frame, Wheat.monarchs giving the
  reigns. Updated examples to more closely approximate Playfair’s
  original and a revision.
...
- Added ZeaMays data from Darwin, used by Fisher for an early
  non-parametric test
- Illustrated permutation and non-parametric tests for ZeaMays data
- Added examples for Macdonell data showing contour plots and
  simulations \[thx: Jim Hanley\]
...
CRAN release: 2011-10-10

- Added ChestSizes data - Quetelet’s data on chest measurements of 5738
  Scottish Militiamen
