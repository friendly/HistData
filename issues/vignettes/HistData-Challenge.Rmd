...
Citations now use [@key] form, resolved against histdata.bib (apa.csl copied from vcdExtra's
vignettes/ for consistent APA-style formatting/citation-linking across packages).

When this is ready to ship:
  1. Switch to a proper vignette YAML header (rmarkdown::html_vignette + %\VignetteIndexEntry
     etc.) -- bibliography/csl wiring above can stay as-is, html_vignette respects the same
     fields.
  2. git mv issues/vignettes/HistData-Challenge.Rmd vignettes/HistData-Challenge.Rmd (bring
     histdata.bib and apa.csl along too)
  3. devtools::document(); devtools::check(); NEWS.md entry; rebuild pkgdown
...
annual count. But Sandy Zabell [-@Zabell:1976] found that the values for 1674 and 1704 are identical --
...
tabulation alone hides. Wainer and Spence [-@WainerSpence:2005:longitudinal] give the fuller account and push the point
...
See Zabell [-@Zabell:1976] for the original discovery, and Zabell and Wainer
[-@ZabellWainer:2002] and Wainer and Spence [-@WainerSpence:2005:longitudinal] for the fuller
story.
...
Galton actually arrived at correlation, and "Regression Towards the Mean, Historically
Considered" [-@Stigler:1997] does the same for Galton's geometric argument for regression --
reconstructing the reasoning behind a graphic, not just restating its result.

David Bellhouse (with Laura Murray) does something similar for a much earlier and more
painstaking case: Murray and Bellhouse [-@MurrayBellhouse:2017] work out how Edmond Halley actually constructed his
...
- Hanley, Julien, and Moodie [-@Hanley:2008], "Student's z, t, and s: What if Gosset had R?," traces the
...
along the march route as time advances.

![Ron Kennett's moving-bubble re-vision of Minard's march](figures/Kennet-moving-bubble.gif){width=90%}
...
The rest of Friendly's [-@Friendly:2002] catalog isn't reproduced here (some entries are third-party work
...
TODO: Mention the pecuilar drop off / Cite Friendly, Wainer \S 8.4
A peculiar feature in this graph is the dip in younger people from about 1850–1870—it looks like
someone chipped off a large chunk of the mountain. What could be the reason? Several demographer
friends speculated: perhaps a war or an outbreak of disease? It turns out that the probable explanation is
less dramatic: Sweden had large outmigration in the period.

<!--
![Perozzo's original 1880/1881 (Wikimedia Commons, public domain)](figures/perozzo-original.jpg){width=70%}
-->

![Perozzo's 1881 stereogram](figures/Perozzo-600dpi.jpg){width=60%}
...
TODO: Not sure this is worth it. I did a better job of showing the 2D densities; but not sure
  where to find the code/image for this. A version of this is available somewhere in
  C:\Dropbox\Twitter\twitter-archive-2022-11-06\data\tweets_media
...
Generated automatically from `histdata.bib` (APA style, via `apa.csl`) for every `[@key]`
citation used above.
