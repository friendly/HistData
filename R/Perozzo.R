...
#'
#' The original numbers were published as a printed table in Perozzo's own 1880 paper (see
#' `@source`), not read off the stereogram image itself. That table is organized by
#' birth-year cohort (`Anni di nascita`, in 5-year bands) crossed with age at observation --
#' not by census year x age directly. The `Year` column here (census year) comes from a
#' `birth_year + Age` transform applied after digitizing the table, not from the original
#' table's own layout.
#'
#' The full chain is now documented: RJ Andrews OCR'd the printed table from a HathiTrust
#' scan, hand-corrected several OCR misreads, transformed the birth-cohort table into this
#' `Year` x `Age` grid, and tidied it to long format (confirmed directly with him,
#' 2026-08-31). That result was posted, byte-identical to the copy shipped here, as
#' `porozzo-tidy.csv` in his `old-charts` GitHub repo -- the route this package's copy was
#' traced through as of 2026-08-07, before the fuller upstream story was confirmed with him
#' directly. See [issues/vignettes/verifying-perozzo.md](https://github.com/friendly/HistData/blob/master/issues/vignettes/verifying-perozzo.md)
#' (on GitHub -- `issues/` is dev-only and not included in the installed package) for the
#' full trace.
#'
...
#'
#' @source
#' Traced to the original printed table in Perozzo's own 1880 paper, *Annali di Statistica*
#' ser. 2, v. 11-12 (1880), scanned by HathiTrust:
#' <https://babel.hathitrust.org/cgi/pt?id=mdp.39015033949978&seq=226>. RJ Andrews OCR'd and
#' corrected that table, then transformed it into this `Year` x `Age` grid (see Details).
#' His result was posted, byte-identical to this package's copy, as `porozzo-tidy.csv` in
#' his `old-charts` GitHub repo:
#' <https://github.com/infowetrust/old-charts/tree/main/src/components/PerozzoSweden/data>
#' (see also <https://charts.infowetrust.com/>). The original stereogram -- Perozzo's
#' graphic built from this same table -- is held by Wikimedia Commons:
